# -*- coding: utf-8 -*-
"""
Created on Wed Aug  3 13:13:50 2022
...
import sys
sys.path.append('/Users/murep/Desktop/논문 작업')

from mgwr.kernels import *
from mgwr.summary import *
from mgwr.diagnostics import get_AIC, get_AICc, get_BIC, corr
...
class GWR_W(GLM):
    ...
    def __init__(self, coords, y, X, bw, family=Gaussian(), offset=None,
                 ...
                 spherical=False, hat_matrix=False, W=None):
        """
        Initialize class
        """
        ...
        if W is None:
            self.W = np.zeros(self.n*self.n).reshape(self.n,self.n)
        else:
            self.W = W
        self.W = W


    def _build_wi(self, i, bw, W):
    ...
    def _local_fit(self, i):
        ...
        """
        wi = self._build_wi(i, self.bw, self.W).reshape(-1, 1)  #local spatial weights
    ...
    def fit(self, ini_params=None, tol=1.0e-5, max_iter=20, solve='iwls',
        ...
        if solve.lower() == 'iwls':
            ...
            if lite:
                return GWRResultsLite2(self, resid, influ, params)
            else:
                ...
                return GWRResults2(self, params, predy, S, CCT, influ, tr_STS,
                                  w)
...
class GWRResults2(GLMResults):
    ...
    def W(self):
        W = np.array(
            [self.model._build_wi(i, self.model.bw, self.model.W) for i in range(self.n)])
        return W
    ...
    def y_bar(self):
        ...
        for i in range(n):
            w_i = np.reshape(self.model._build_wi(i, self.model.bw, self.model.W), (-1, 1))
    ...
    def TSS(self):
        ...
        for i in range(n):
            TSS[i] = np.sum(
                np.reshape(self.model._build_wi(i, self.model.bw, self.model.W),
                           (-1, 1)) * (self.y.reshape(
                               (-1, 1)) - self.y_bar[i])**2)
        return TSS
    ...
    def local_collinearity(self):
        ...
        for i in range(nrow):
            wi = self.model._build_wi(i, self.model.bw, self.model.W)
            sw = np.sum(wi)
            wi = wi / sw
            tag = 0
...
class MGWR_W(GWR_W):
    ...
    def __init__(self, coords, y, X, selector, sigma2_v1=True,
        ...
        GWR_W.__init__(self, coords, y, X, self.bw_init, family=self.family,
    ...
    def fit(self, n_chunks=1, pool=None):
        ...
        if self.hat_matrix:
            ...
            R = None
        return MGWRResults2(self, params, predy, CCT, ENP_j, w, R)
    ...
    def exact_fit(self):
        ...
        for j1 in range(self.k):
            Aj = GWR_W(self.coords,self.y,self.X[:,j1].reshape(-1,1),bw=self.bws[j1],hat_matrix=True,constant=False).fit().S
            Pj = []
...
class MGWRResults2(GWRResults2):
    """
    ...
    def __init__(self, model, params, predy, CCT, ENP_j, w, R):
        ...
        self.ENP_j = ENP_j
        self.R = R
        GWRResults2.__init__(self, model, params, predy, None, CCT, None, w)
        if model.hat_matrix:
